...
int countRowShift(int  *rows_number, int process_rank, int firstCalcProcRank);
...
int main(int argc, char* argv[]){
    ...
    int reminder = dimension%(size - firstCalcProcRank);

    int * rows_number = new int[size-firstCalcProcRank];

    for (int i = 0 ; i< size - firstCalcProcRank; i++){
        if( i< reminder)
            rows_number[i] = process_rows_number + 1;
        else 
            rows_number[i] = process_rows_number; 
    }

    process_rows_number = rows_number[process_rank - firstCalcProcRank];
    ...
    if (!openfile.good()) {
        ...
        // printf("height %d\n", process_rows_number);
        int rows_shift = countRowShift(rows_number, process_rank, firstCalcProcRank);
        if (saveOutput){
            if (process_rank > 0){
                openfile.seekg(((dimension * 2)) * rows_shift ,ios::beg);
            }
        }
        else{
            openfile.seekg(((dimension * 2)) * rows_shift ,ios::beg);  // przesunięcie o 1 linie = dlugosc*2
        }    
        int tmp;
        for (int i = 0; i < process_rows_number; i++)
        {
    ...
    if (saveOutput)
    {
        if (process_rank == 0)
        {
            ...
            for (int i = 1; i < size; i++){
                ...
                for (int j = 0; j < rows_number[i-firstCalcProcRank]; j++){
...
int countRowShift(int *rows_number, int process_rank, int firstCalcProcRank){
    int rows_shift = 0;
    for (int i = 0; i< process_rank-firstCalcProcRank; i++){
        rows_shift+= rows_number[i];
    }
    return rows_shift;
}
